...
# An exploration is missing data
```{r more-school-data}
...
glimpse(more_school_data) 
# 182 schools

# This will show you any rows where enrollment is NA. 
more_school_data %>% 
    filter(if_any(enrollment, ~ is.na(.x))) %>% 
    glimpse()
#shows our two empty schools: New Southeast Elementary Relief School and Olympic High Relief School

intermed_old_race <- read_csv('edp_school_race.csv', col_types = cols(.default="?", "ncessch" = col_character())) %>% 
    pivot_wider(names_from = race, values_from = enrollment) %>% 
    clean_names() %>% 
    glimpse()
# 180 schools

intermed_new_race <- read_csv('EducationDataPortal_05.16.2024_race.csv', col_types = cols(.default="?", "ncessch" = col_character())) %>% 
  pivot_wider(names_from = race, values_from = enrollment) %>% 
  clean_names()
#this contains 360 entries, all race data for 2021 and for 2022. 

closed_schools_race <- intermed_new_race %>% 
  filter(str_detect(school_name,"Mint|Palisades"))
#these schools evidently were open during both 2021 and 2022 collection years. 
#if we switch to 2022 data, both schools will be present on the map.

relief_race <- intermed_new_race %>% 
  filter(str_detect(school_name, "Relief"))
#our relief schools only have data for 2022.
#also, they do have unique ncessch codes!

# are we using 2021 data or 2022 data in the tool? 
race_data_in_tool <- intermed_old_race %>% 
  left_join(intermed_new_race, by = c("ncessch", "school_name", "state_location","white", "black", "hispanic", "asian", "american_indian_or_alaska_native", "native_hawaiian_or_other_pacific_islander","two_or_more_races", "unknown","total")) %>% 
  select(year.x, year.y, ncessch, school_name)
# we are using only 2021 data in the tool, which is why the relief schools dropped off. 
# we can move to 2022 data, but we'll need to re-copyedit the school names. 

new_possible_race_school_data <- intermed_new_race %>% 
  filter(year == 2022)
#this still has only 180 schools present

# but if there are 180 schools with race data including our two relief schools and their neighbors,
# why does our non-race school data include 182 entries?
schools_missing_race_data <- more_school_data %>% 
  anti_join(new_possible_race_school_data, by = c("ncessch" = "ncessch"))
# the virtual middle and elementary schools don't have race data for 2022, only 2021
# did they close because they were temporary for covid?
# the high school was still open, tho
```



# Takeaways from data explorations:
If we update our data from 2021 data to 2022 data, we'll be adding our two relief schools
that just opened and losing two virtual schools that no longer operate.





# Building out old race data
```{r}
...
glimpse(race_school_data)


# Run merge test
more_school_data %>% 
  anti_join(race_school_data, by=c("ncessch" = "ncessch")) %>% 
    glimpse()
# These two schools didn't join

school_specific_data <- more_school_data %>% 
  left_join(race_school_data, by=c("ncessch" = "ncessch")) %>% 
  st_as_sf(coords = c("longitude", "latitude"),
            crs = st_crs(tract_set))
glimpse(school_specific_data)

# This will show you all rows where any column between enrollment and share_other
# has an NA present
## a lot of these columns did not go into the tool. 
school_specific_data %>% 
    filter(if_any(enrollment:share_other, ~ is.na(.x))) %>% 
    glimpse() # 26 rows
